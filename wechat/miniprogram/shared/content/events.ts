...
import type { Event, ResourceState } from '../types/game';
...
/** 按角色 ID 索引事件 */
export const EVENTS_BY_CHARACTER: Readonly<Record<string, Event[]>> = (() => {
  const map: Record<string, Event[]> = {};
  for (const event of EVENTS) {
    if (!map[event.character_id]) map[event.character_id] = [];
...
/** 通用事件（找不到角色事件时的回退） */
export const GENERIC_EVENT: Readonly<Event> = {
  event_id: 'EVT_GENERIC',
  character_id: '',
  name: '临时任务压迫',
  base_effect: { hp: 0, en: -8, st: -4, kpi: 0, risk: 2, cor: 0 } as ResourceState,
};
