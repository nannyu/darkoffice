...
function tickHazards(hazards) {
    ...
    for (const hazard of hazards) {
        const current = Object.assign(Object.assign({}, hazard), { countdown: hazard.countdown - 1 });
...
function tickProjects(projects, actionType) {
    ...
    for (const project of projects) {
        const current = Object.assign({}, project);
...
function pickEvent(characterId, ctx) {
    ...
    if (pool.length === 0) {
        return { eventId: events_1.GENERIC_EVENT.event_id, name: events_1.GENERIC_EVENT.name, baseEffect: Object.assign({}, events_1.GENERIC_EVENT.base_effect) };
    }
    ...
    for (const [event, w] of weighted) {
        r -= w;
        if (r <= 0) {
            return { eventId: event.event_id, name: event.name, baseEffect: Object.assign({}, event.base_effect) };
        }
    }
    const last = pool[pool.length - 1];
    return { eventId: last.event_id, name: last.name, baseEffect: Object.assign({}, last.base_effect) };
}
function resolveTurn(input) {
    ...
    if (maybeHazard && !hazards.some(h => h.id === maybeHazard.id)) {
        hazards.push(Object.assign({}, maybeHazard));
    }
    ...
    const nameMap = Object.assign(Object.assign({}, characters_1.CHARACTER_NAME_MAP), ctx.characterNameMap);
    const nextTimePeriodId = timePeriod(newTurn + 1);
    const nextCharacterId = pickCharacter(Object.assign(Object.assign({}, session), { turn_index: newTurn, state: newClampedState }), Object.assign(Object.assign({}, ctx), { previousCharacterId: characterId, previousEventId: event.eventId }), nextTimePeriodId);
    const nextEvent = pickEvent(nextCharacterId, Object.assign(Object.assign({}, ctx), { previousCharacterId: characterId, previousEventId: event.eventId }));
