...
import type { TimePeriodRule } from '../types/game';
import { TURNS_PER_DAY } from './resources';

export const TIME_PERIOD_RULES: readonly TimePeriodRule[] = [
...
/** 按回合数获取当前时间段定义 */
export function timePeriodForTurn(turnIndex: number): TimePeriodRule {
  const dayTurn = turnIndex % TURNS_PER_DAY;
  for (const rule of TIME_PERIOD_RULES) {
    if (!rule.enabled) continue;
    if (rule.turn_start <= dayTurn && dayTurn <= rule.turn_end) {
      return { ...rule, weight_modifiers: { ...rule.weight_modifiers } };
    }
  }
  const fallback = TIME_PERIOD_RULES[0];
  return { ...fallback, weight_modifiers: { ...fallback.weight_modifiers } };
}

/** 获取指定时间段 ID 对应的角色权重修正表 */
export function timePeriodWeightModifiers(timePeriod: string): Record<string, number> {
  for (const rule of TIME_PERIOD_RULES) {
    if (rule.id === timePeriod) {
      return { ...rule.weight_modifiers };
    }
  }
  return { ...TIME_PERIOD_RULES[0].weight_modifiers };
}
