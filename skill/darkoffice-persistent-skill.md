...
**触发关键词**：
- 明确求助：领导/同事/晋升/站队/背锅/甩锅/加班/绩效
- 隐含需求：工作烦恼、职场困惑、不知道怎么办
...
- 峰video思维框架驱动
- 情境分析 + 选项生成 + 后果预测
- 参考 `skill/advisor/darkoffice-advisor-skill.md`
...
**主题**：温水煮青蛙式的权力围猎。从乡镇教师到国企董事长，商人李某用"人情"瓦解防线，副市长介入施压，最终纪委调查——你能及时醒悟，还是终将自毁？

**分支结构**：

```
幕0（温水）— 商人李某红包试探
  ├─ 分支A: EMAIL_TRACE/BOUNDARY_RESTATE + SUCCESS → 幕10（清醒路线）
  └─ 分支B: COR≥5 → 幕1（沦陷路线）

清醒路线：幕10（留痕）→ 幕11（反思）→ 幕12（终局）→ 结局 AWAKENED
沦陷路线：幕1（围猎）→ 幕2（捆绑）→ 幕3（裂痕）→ 幕4（坠落）→ 结局 FALLEN
黑化加速：幕1 SHIFT_BLAME + COR≥20 → 幕3（跳过捆绑）
黑化掌权：幕3 COR≥50 → 幕20（黑化掌权）→ 结局 BLACKENED
```
...
| 结局ID | 名称 | 类型 | 触发条件 |
|--------|------|------|---------|
| AWAKENED | 幡然醒悟 | 好结局 | COR≤15，回合≥5，历史使用过 EMAIL_TRACE/BOUNDARY_RESTATE |
| FALLEN | 彻底坠落 | 坏结局 | COR≥40，RISK≥40，回合≥5 |
| BLACKENED | 黑化掌权 | 黑化结局 | COR≥70，回合≥5 |
| SUICIDE | 坠楼身亡 | 坏结局 | HP≤20，COR≥30，回合≥5 |

**自定义角色卡**（已内置激活）：
- `CUSTOM_CHR_BUSINESSMAN` 商人李某（围猎者）
- `CUSTOM_CHR_MAYOR` 副市长杨（权力庇护者）
- `CUSTOM_CHR_INSPECTOR` 纪委调查员（终局审判者）

**自定义事件卡**（已内置激活）：
- `CUSTOM_EVT_01` 过节了，一点心意
- `CUSTOM_EVT_02` 有个项目想请你关照
- `CUSTOM_EVT_03` 大家一起赚，算你一份
- `CUSTOM_EVT_04` 咱们是一条船上的人
- `CUSTOM_EVT_05` 这个项目你要支持一下
- `CUSTOM_EVT_06` 有些事别深究
- `CUSTOM_EVT_07` 请配合组织调查
- `CUSTOM_EVT_08` 这些记录你怎么解释
...
3. **展示选择**（仅当剧情库非空时）：
   ...
   1. 「围猎：董事长的坠落」(SL_HUNT) — 从红包破冰到项目黑洞，一路深陷围猎漩涡
